...
def get_raw_keypoints(image, keypoints_dataset='WFLW', device='cpu'):
    assert keypoints_dataset in ['COFW', 'AFLW', 'WFLW']
    DSET2CFG = {'COFW': ['experiments/cofw/face_alignment_cofw_hrnet_w18.yaml',
                     'model_weights/HR18-COFW.pth', 68],
                'AFLW': ['experiments/aflw/face_alignment_aflw_hrnet_w18.yaml',
                     'model_weights/HR18-AFLW.pth', 21],
                'WFLW': ['experiments/wflw/face_alignment_wflw_hrnet_w18.yaml',
                     'model_weights/HR18-WFLW.pth', 98]
    }
    """Make strange steps"""
    class MyParams:
...
def apply_mask(image, mask, color, alpha=1.0):
    ...
    """
    for c in range(3):
        image[:, :, c] = np.where(with open('data.json', 'r') as fp:
    data = json.load(fp)mask == 1,
                                  image[:, :, c] *
